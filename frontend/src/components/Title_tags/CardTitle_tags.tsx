...
const CardTitle_tags = ({
  ...
  return (
    <div className={'p-4'}>
      ...
      >
        {!loading && title_tags.map((item, index) => (
          <li
            key={item.id}
          ...
          >
            ...
            <div className={`flex items-center ${bgColor} p-6  gap-x-4 border-b border-gray-900/5 bg-gray-50 dark:bg-dark-800 relative`}>
                ...
                <Link href={`/title_tags/title_tags-view/?id=${item.id}`} className='text-lg font-bold leading-6 line-clamp-1'>
                    {item.title}
                </Link>
