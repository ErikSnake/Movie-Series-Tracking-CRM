...
const CardAttachments = ({
  ...
  return (
    <div className={'p-4'}>
      ...
      >
        {!loading && attachments.map((item, index) => (
          <li
            key={item.id}
          ...
          >
            ...
            <div className={`flex items-center ${bgColor} p-6  gap-x-4 border-b border-gray-900/5 bg-gray-50 dark:bg-dark-800 relative`}>
                ...
                <Link href={`/attachments/attachments-view/?id=${item.id}`} className='text-lg font-bold leading-6 line-clamp-1'>
                    {item.name}
                </Link>
            ...
            <dl className='divide-y  divide-gray-600   dark:divide-dark-700 px-6 py-4 text-sm leading-6 h-64 overflow-y-auto'>
                ...
                <div className='flex justify-between gap-x-4 py-3'>
                    <dt className='  text-gray-500  dark:text-dark-600'>AttachedAt</dt>
                    <dd className='flex items-start gap-x-2'>
                        <div className='font-medium line-clamp-4'>
