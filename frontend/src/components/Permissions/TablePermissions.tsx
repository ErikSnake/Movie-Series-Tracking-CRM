...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/permissions/permissionsSlice'
...
const TableSamplePermissions = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { permissions, loading, count, notify: permissionsNotify, refetch } = useAppSelector((state) => state.permissions)
  ...
  useEffect(() => {
      if (permissionsNotify.showNotification) {
          notify(permissionsNotify.typeNotification, permissionsNotify.textNotification);
      }
